...
static ssize_t sysfs_fps_read(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	if (!display->panel)
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n", display->panel->fps_mode);
}

static ssize_t sysfs_fps_write(struct device *dev,
	    struct device_attribute *attr, const char *buf, size_t count)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	int ret, fps_mode;
	...
	display->panel->fps_mode = fps_mode;
	if (!dsi_panel_initialized(display->panel)) {
		goto error;
	}
	...
	ret = dsi_panel_apply_fps_mode(display->panel);
	if (ret)
		pr_err("unable to set fps mode\n");
...
static DEVICE_ATTR(fps, 0644, sysfs_fps_read, sysfs_fps_write);
