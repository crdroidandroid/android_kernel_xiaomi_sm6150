		...
		uint32_t stage)
{
	...
	alpha =
	    dsi_panel_get_dim_alpha(display->panel, display->panel->fod_dim_lut,
				    display->panel->fod_dim_lut_count);
	if (is_dimlayer_bl_enabled && !is_dimlayer_hbm_enabled) {
		alpha = display->panel->doze_enabled
			    ? 0
			    : dsi_panel_get_dim_alpha(
				  display->panel, display->panel->bl_dim_lut,
				  display->panel->bl_dim_lut_count);
	}
...
static void sde_crtc_fod_atomic_check(struct sde_crtc_state *cstate,
	...
	if (is_dimlayer_hbm_enabled || is_dimlayer_bl_enabled) {
		...
		for (plane_idx = 0; plane_idx < cnt; plane_idx++) {
			if (plane_idx == fod_layer_index) continue;
		...
		if (!cstate->fod_dim_layer) return;
	} else {
