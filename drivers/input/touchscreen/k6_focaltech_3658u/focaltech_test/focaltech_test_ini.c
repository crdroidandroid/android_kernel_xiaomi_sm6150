...
static void print_ini_data(struct ini_data *ini)
{
	...
	FTS_TEST_DBG("section num:%d, keyword num total:%d", ini->section_num, ini->keyword_num_total);
	for (i = 0; i < ini->section_num; i++) {
		section = &ini->section[i];
		FTS_TEST_DBG("section name:[%s] keyword num:%d", section->name, section->keyword_num);
...
static int ini_parse_section(struct ini_data *ini, char *line_buffer)
{
	...
	FTS_TEST_INFO("section:%s, keyword offset:%d", section->name, ini->keyword_num_total);
		...
		for (j = 0; j < section->keyword_num; j++) {
			...
			if (key_len == strlen(keyword->name)) {
				if (0 == fts_strncmp(keyword->name, key_name, key_len)) {
					...
					if (log_level >= 3) {
						FTS_TEST_DBG("section:%s,%s=%s", section_name, key_name, value);
					}
