...
static ssize_t fts_auto_fwupdate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	logError(1,"%s Update firmware success %08X\n", tag, ret);
	return snprintf(buf, PAGE_SIZE, "{ %08X }\n", ret);
}
#endif
...
/*
	if (fts_touchmode_edgefilter(touchId, x, y))
		return;
*/
#endif
	...
	if (event[0] == 0x13)
		logError(1,
			"%s  %s :  Event 0x%02x - Press ID[%d] Fod finger ID = %d type = %d\n", tag,
				__func__, event[0], touchId, info->fod_id, touchType);
...
*/
	...
	if (fod_up)
		...
		logError(1, "%s  %s :  Event 0x%02x - release ID[%d] type = %d\n", tag,
			...
			if ((!info->sensor_sleep && info->fod_coordinate_update &&
				...
				if ((info->sensor_sleep && !info->sleep_finger) || !info->sensor_sleep) {
					...
					if (info->fod_coordinate_update || info->sensor_sleep) {
						...
						logError(1, "%s %s id:%d touch_area:%d, overlap:%d,fod report\n",
...
bool inline fts_touchmode_edgefilter(unsigned int touch_id, int x, int y)
{
	...
	switch (xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][GET_CUR_VALUE]) {
		case 0:
			filter_size = 0;
			break;
		case 1:
		...
		default:
			filter_size = 0;
			logError(1, "%s %s: no support value %08d\n", tag, __func__, filter_size);
			break;
	}
	...
	if (test_bit(touch_id, &fts_info->touch_skip)) {
		logError(1, "%s %s: skip mask finger:%d, %x\n",
					tag, __func__, touch_id, fts_info->touch_skip);
		return true;
	} else if ((x >= 0 && x < filter_size) && ((y >= 0 && y < filter_size) ||
			(y > bdata->y_max - filter_size && y < bdata->y_max))) {

		if (y >= 0 && y < filter_size) {
			top_half = true;
		} else {
			top_half = false;
		}
		...
		for (i = 0; i < TOUCH_ID_MAX; i++) {
			if ((i == touch_id) || (!test_bit(i, &fts_info->touch_id)))
				continue;
			if ((fts_info->coor[i][1] > (int)(bdata->y_max / 2) && top_half) ||
				(fts_info->coor[i][1] < (int)(bdata->y_max / 2) &&
				fts_info->coor[i][1] >= 0 && !top_half)) {
				return false;
			}
		}

		if (!test_bit(touch_id, &fts_info->touch_id) && fts_info->touch_id != 0) {
			__set_bit(touch_id, &fts_info->touch_skip);
			logError(1, "%s %s: skip mask first finger:%d, touch_id:%x, touch_skip:%0x\n",
					tag, __func__, touch_id, fts_info->touch_id, fts_info->touch_skip);
			return true;
		}
	}
...
int fts_read_touchmode_data()
{
	...
	ret = fts_writeRead_dma_safe(grip_cmd, sizeof(grip_cmd) / sizeof(u8), grip_value,
			     readBytes);
	if (ret < OK) {
		logError(1,
			 "%s %s: error while reading edge filter data ERROR %08X\n",
...
static void fts_edge_rejection(bool on, int top, int bottom)
{
	/* the fixed edge_reject is 65 */
	int ret = 0;
	u8 edge_of_leftright[4] = {0xc0, 0x01, 0x00, 0x00};
	u8 edge_of_top[4] = {0xc0, 0x01, 0x02, 0x00};
	u8 edge_of_bottom[4] = {0xc0, 0x01, 0x03, 0x00};
	...
	if (on) {
		...
		/*add four edge inhibition when in non-game mode and direction is 1/3*/
		if (xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][SET_CUR_VALUE]) {
			edge_of_top[3] = pdata->edgefilter_top_ingamemode;
			edge_of_bottom[3] = pdata->edgefilter_bottom_ingamemode;
		} else {
			edge_of_top[3] = pdata->edgefilter_topbottom_def;
			edge_of_bottom[3] = pdata->edgefilter_topbottom_def;
		}
		edge_of_leftright[3] = pdata->edgefilter_leftright_def;
	}

	ret = fts_write_dma_safe(edge_of_leftright, sizeof(edge_of_leftright) / sizeof(u8));
	if (ret < OK) {
		logError(1,
	...
	ret = fts_write_dma_safe(edge_of_top, sizeof(edge_of_top) / sizeof(u8));
	if (ret < OK) {
		logError(1,
			 "%s %s: error while writing edge top data ERROR %08X\n",
			 tag, __func__, ret);
		return;
	}
	ret = fts_write_dma_safe(edge_of_bottom, sizeof(edge_of_bottom) / sizeof(u8));
	if (ret < OK) {
		logError(1,
			 "%s %s: error while writing edge bottom data ERROR %08X\n",
			 tag, __func__, ret);
		return;
	}
	logError(1, "%s %s: direction:%d, top:%d, bottom:%d\n", tag, __func__,
		xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][SET_CUR_VALUE],
		edge_of_top[3], edge_of_bottom[3]);
...
static void fts_update_enter_idle_time(void)
{
	...
	if (fts_info->gamemode_enable) {
		logError(1, "%s %s is in gamemode, set idle time to default value\n", tag, __func__);
	...
	if (ret < OK) {
		...
		logError(1, "%s %s write enter idle time to :%d\n", tag, __func__, value);
	return;
}
...
{
	int i, ret;
	u8 grip_cmd[8] = {0xc0, 0x08, 0x00,};
	u8 grip_rcmd[2] = {0xc1, 0x08};
	u8 grip_value[6] = {0x0,};
	int x_filter_value = 0, y_filter_value = 0;
	int direction = 0;
	...
	switch (xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][SET_CUR_VALUE]) {
		case 0:
			...
			break;
		case 1:
			x_filter_value = pdata->edgefilter_area_step1;
			y_filter_value = pdata->edgefilter_area_step1;
			break;
		case 2:
			x_filter_value = pdata->edgefilter_area_step2;
			y_filter_value = pdata->edgefilter_area_step2;
			break;
		case 3:
			x_filter_value = pdata->edgefilter_area_step3;
			y_filter_value = pdata->edgefilter_area_step3;
			break;
		default:
			x_filter_value = 100;
			y_filter_value = 150;
	...
	if (!fts_info->gamemode_enable) {
		if (direction == 1 || direction == 3) {
			x_filter_value = pdata->nongame_horizontal_corner_threshold;
			y_filter_value = pdata->nongame_horizontal_corner_threshold;
		} else {
			x_filter_value = pdata->nongame_vertical_corner_threshold;
			y_filter_value = 2 * pdata->nongame_vertical_corner_threshold;
		}
	}
	/*
	*touch_edge[0] : game mode enable/disable
	*touch_edge[1] : panel direction 0/1/2/3
	*touch_edge[2] : y direction threshold low 8 bit
	*touch_edge[3] : y direction threshold high 8 bit
	*touch_edge[4] : x direction threshold low 8 bit
	*touch_edge[5] : x direction threshold high 8 bit
	*/
	xiaomi_touch_interfaces.touch_edge[0] = fts_info->gamemode_enable;
	xiaomi_touch_interfaces.touch_edge[1] = direction;
	xiaomi_touch_interfaces.touch_edge[2] = (y_filter_value & 0xff);
	xiaomi_touch_interfaces.touch_edge[3] = ((y_filter_value >> 8) & 0xff);
	xiaomi_touch_interfaces.touch_edge[4] = (x_filter_value & 0xff);
	xiaomi_touch_interfaces.touch_edge[5] = ((x_filter_value >> 8) & 0xff);

	if (!xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][GET_CUR_VALUE]) {
		logError(1, "%s %s: touch game mode off, enter non-game mode\n", tag, __func__);
		fts_edge_rejection(false, 0, 0);
	} else
		fts_edge_rejection(true, pdata->edgefilter_top_ingamemode, pdata->edgefilter_bottom_ingamemode);
	...
	logError(1, "%s %s:game mode:%d, direction:%d, y_low:%02X, y_high:%02X, x_low:%02X, x_high:%02X\n",
		tag, __func__, grip_cmd[2], grip_cmd[3], grip_cmd[4], grip_cmd[5], grip_cmd[6], grip_cmd[7]);
	...
	logError(1,
		"%s %s: read grip:0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
		tag, __func__, grip_value[0], grip_value[1], grip_value[2],
		grip_value[3], grip_value[4], grip_value[5]);
...
{
	char value = '1';
	logError(1, "%s Get touch vendor: %c\n", __func__, value);
	return value;
}
				      ...
				      unsigned long val, void *data)
{
	...
	if (evdata && evdata->data && info) {
		...
		if (val == MSM_DRM_EVENT_BLANK && blank == MSM_DRM_BLANK_POWERDOWN) {
			if (info->sensor_sleep)
				return NOTIFY_OK;
...
static int parse_gamemode_dt(struct device *dev, struct fts_hw_platform_data *bdata)
{
	...
	retval = of_property_read_u32(np, "fts,edgefilter-top-ingamemode", &temp_val);
	if (retval < 0)
		return retval;
	else
		bdata->edgefilter_top_ingamemode = temp_val;

	retval = of_property_read_u32(np, "fts,edgefilter-bottom-ingamemode", &temp_val);
	if (retval < 0)
		return retval;
	else
		bdata->edgefilter_bottom_ingamemode = temp_val;

	retval = of_property_read_u32(np, "fts,vertical-corner-threshold", &temp_val);
		if (retval < 0)
			return retval;
		else
			bdata->nongame_vertical_corner_threshold = temp_val;

	retval = of_property_read_u32(np, "fts,horizontal-corner-threshold", &temp_val);
		if (retval < 0)
			return retval;
		else
			bdata->nongame_horizontal_corner_threshold = temp_val;
...
static int fts_probe(struct spi_device *client)
{
	...
	if (IS_ERR(info->fts_touch_dev)) {
		logError(1, "%s Failed to create device for the sysfs!\n", tag);
	...
	if (error)
		logError(1, "%s Failed to create fod_test sysfs group!\n", tag);
#endif
