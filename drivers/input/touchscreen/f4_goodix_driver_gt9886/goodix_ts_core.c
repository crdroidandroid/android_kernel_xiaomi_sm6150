...
static void __do_register_ext_module(struct work_struct *work)
{
	...
	ts_info("__do_register_ext_module IN, goodix_modules.core_exit:%d",
					  ...
					  char *buf)
{
	...
	if (ncfg && ncfg->initialized && ncfg->length < PAGE_SIZE) {
		...
		r = ts_dev->hw_ops->read(ts_dev, ncfg->reg_base, &data[0],
					 ncfg->length);
		if (r < 0) {
			kfree(data);
			return -EINVAL;
		...
		for (i = 0; i < ncfg->length; i++) {
			if (i != 0 && i % 20 == 0)
				buf[offset++] = '\n';
			offset +=
			    snprintf(&buf[offset], PAGE_SIZE - offset, "%02x ",
				     data[i]);
		}
		buf[offset++] = '\n';
		buf[offset++] = '\0';
	...
	if (ret > 0) {
		for (i = 0; i < ret; i++) {
			if (i != 0 && i % 20 == 0)
				buf[offset++] = '\n';
			offset +=
			    snprintf(&buf[offset], 4096 - offset, "%02x ",
				     cfg_buf[i]);
		}
	}
	kfree(cfg_buf);
...
static int goodix_ts_convert_0x_data(const u8 *buf,
				     int buf_size, unsigned char *out_buf,
				     int *out_buf_len)
{
	int i, m_size = 0;
	int temp_index = 0;
	...
	for (i = 0; i < buf_size; i++) {
		if (buf[i] == 'x' || buf[i] == 'X') {
			if (temp_index >= m_size) {
				ts_err
				    ("exchange cfg data error, overflow, temp_index:%d,m_size:%d\n",
				     temp_index, m_size);
				return -EINVAL;
			}
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	if (core_data->avdd) {
		if (core_data->avdd_load) {
			r = regulator_set_load(core_data->avdd,
					       core_data->avdd_load);
			if (r < 0)
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data)
{
	...
	r = devm_gpio_request_one(&core_data->pdev->dev,
				  ts_bdata->irq_gpio, GPIOF_IN, "ts_irq_gpio");
	if (r < 0) {
...
out:
	...
	ts_info("delay 500 ms\n");
	mdelay(500);
	gtp_set_cur_value(Touch_Active_MODE, 0);
#endif
...
static int goodix_bl_state_chg_callback(struct notifier_block *nb,
					 ...
					 size_t count, loff_t *pos)
{
	...
	if (*pos != 0 || !goodix_core_data)
		return 0;
	k_buf = kzalloc(4096*sizeof(char), GFP_KERNEL);
	if (k_buf == NULL) {
...
static int gtp_power_supply_event(struct notifier_block *nb,
...
static void goodix_set_grip_filter(int *source, u8 *result, int *sum)
{
	...
	*sum = 0;
/*	for(i = 0; i < 4; i++){
		ts_info("i = %d source(int): %d, %d, %d, %d, %d, %d, %d, %d", i,
				source[0 + i * GTP_PARAMETER_NUM], source[1 + i * GTP_PARAMETER_NUM],
				source[2 + i * GTP_PARAMETER_NUM], source[3 + i * GTP_PARAMETER_NUM],
				source[4 + i * GTP_PARAMETER_NUM], source[5 + i * GTP_PARAMETER_NUM],
				source[6 + i * GTP_PARAMETER_NUM], source[7 + i * GTP_PARAMETER_NUM]);
	}*/
	...
	for(i = 0; i < 4; i++){
		...
		x_end = *(buf + 4);
		y_end = *(buf + 5);
		ts_info("i = %d result(int): type: %d, pos: %d, minpos[%d, %d], maxpos[%d, %d] sum_type_pos: %d", i,
				type, pos, x_start, y_start, x_end, y_end, sum_type_pos);
		result[0 + i * GRIP_PARAMETER_NUM] = type;
		result[1 + i * GRIP_PARAMETER_NUM] = pos;
		result[2 + i * GRIP_PARAMETER_NUM] = ((x_start >> 8) & 0xff);
		result[3 + i * GRIP_PARAMETER_NUM] = (x_start & 0xff);
		result[4 + i * GRIP_PARAMETER_NUM] = ((y_start >> 8) & 0xff);
		result[5 + i * GRIP_PARAMETER_NUM] = (y_start & 0xff);
		result[6 + i * GRIP_PARAMETER_NUM] = ((x_end >> 8) & 0xff);
		result[7 + i * GRIP_PARAMETER_NUM] = (x_end & 0xff);
		result[8 + i * GRIP_PARAMETER_NUM] = ((y_end >> 8) & 0xff);
		result[9 + i * GRIP_PARAMETER_NUM] = (y_end & 0xff);
...
/*	for(i = 0; i < 4; i++){
		ts_log("i = %d result(u8): %x, %x, %x, %x, %x, %x, %x, %x, %x, %x", i,
				result[0 + i * GRIP_PARAMETER_NUM], result[1 + i * GRIP_PARAMETER_NUM],
				result[2 + i * GRIP_PARAMETER_NUM], result[3 + i * GRIP_PARAMETER_NUM],
				result[4 + i * GRIP_PARAMETER_NUM], result[5 + i * GRIP_PARAMETER_NUM],
				result[6 + i * GRIP_PARAMETER_NUM], result[7 + i * GRIP_PARAMETER_NUM],
				result[8 + i * GRIP_PARAMETER_NUM], result[9 + i * GRIP_PARAMETER_NUM]);
	}*/
}
...
static void gtp_set_edge_filter_game(int edge_filter_corner_size)
{
	...
	int corner_filter[4 * GTP_PARAMETER_NUM] = {0};
	ts_info("edge_filter_corner_size = %d",edge_filter_corner_size);
	if(!edge_filter_corner_size && direction){
		ts_info("cornerzone 0");
		corner_filter[GTP_PARAMETER_NUM + 1] = 1;
		corner_filter[2 * GTP_PARAMETER_NUM + 1] = 2;
		corner_filter[3 * GTP_PARAMETER_NUM + 1] = 3;
		goodix_set_grip_filter((int*)&(corner_filter[0]),(u8*)&(dev->board_data->edge_filter_params.cornerzone_filter[0]), &sum_corner);
	}else{
		if (direction == PANEL_ORIENTATION_DEGREE_90){
			ts_info("cornerzone hor1");
			dev->board_data->edge_filter_params.cornerzone_filter_hor1[4] = edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor1[5] = edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor1[GTP_PARAMETER_NUM * 2 + 3] = dev->board_data->panel_max_y - edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor1[GTP_PARAMETER_NUM * 2 + 4] = edge_filter_corner_size;
			goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.cornerzone_filter_hor1[0]),(u8*)&(dev->board_data->edge_filter_params.cornerzone_filter[0]), &sum_corner);
		}
		else if (direction == PANEL_ORIENTATION_DEGREE_270){
			ts_info("cornerzone hor2");
			dev->board_data->edge_filter_params.cornerzone_filter_hor2[GTP_PARAMETER_NUM + 2] = dev->board_data->panel_max_x - edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor2[GTP_PARAMETER_NUM + 5] = edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor2[GTP_PARAMETER_NUM * 3 + 2] = dev->board_data->panel_max_x - edge_filter_corner_size;
			dev->board_data->edge_filter_params.cornerzone_filter_hor2[GTP_PARAMETER_NUM * 3 + 3] = dev->board_data->panel_max_y - edge_filter_corner_size;
			goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.cornerzone_filter_hor2[0]),(u8*)&(dev->board_data->edge_filter_params.cornerzone_filter[0]), &sum_corner);
		}
		else{
			ts_info("cornerzone ver");
			goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.cornerzone_filter_ver[0]),(u8*)&(dev->board_data->edge_filter_params.cornerzone_filter[0]), &sum_corner);
		}
	}
	if(direction){
		ts_info("edgezone hor");
		goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.edgezone_filter_hor[0]),(u8*)&(dev->board_data->edge_filter_params.edgezone_filter[0]), &sum_edge);
		ts_info("deadzone hor");
		goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.deadzone_filter_hor[0]),(u8*)&(dev->board_data->edge_filter_params.deadzone_filter[0]), &sum_dead);
	}else{
		ts_info("edgezone ver");
		goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.edgezone_filter_ver[0]),(u8*)&(dev->board_data->edge_filter_params.edgezone_filter[0]), &sum_edge);
		ts_info("deadzone ver");
		goodix_set_grip_filter((int*)&(dev->board_data->edge_filter_params.deadzone_filter_ver[0]),(u8*)&(dev->board_data->edge_filter_params.deadzone_filter[0]), &sum_dead);
	}
	/* calculate checksum */
	dev->board_data->edge_filter_params.check_sum = - (sum_corner + sum_edge + sum_dead);
	ts_log("cornerzone:%*ph", 4 * GRIP_PARAMETER_NUM, dev->board_data->edge_filter_params.cornerzone_filter);
	ts_log("edgezone:%*ph", 4 * GRIP_PARAMETER_NUM, dev->board_data->edge_filter_params.edgezone_filter);
	ts_log("deadzone:%*ph", 4 * GRIP_PARAMETER_NUM, dev->board_data->edge_filter_params.deadzone_filter);
...
static void gtp_set_edge_filter_normal()
{
	...
	if(xiaomi_touch_interfaces.long_mode_value[0] == DEAD_ZONE_TYPE){
		ts_info("deadzone");
		goodix_set_grip_filter((int*)&(xiaomi_touch_interfaces.long_mode_value[0]),(u8*)&(dev->board_data->edge_filter_params.deadzone_filter[0]), &sum_dead);
		ts_info("edgezone");
		goodix_set_grip_filter((int*)&(xiaomi_touch_interfaces.long_mode_value[4 * GTP_PARAMETER_NUM]),(u8*)&(dev->board_data->edge_filter_params.edgezone_filter[0]), &sum_edge);
	} else {
		ts_info("edgezone");
		goodix_set_grip_filter((int*)&(xiaomi_touch_interfaces.long_mode_value[0]),(u8*)&(dev->board_data->edge_filter_params.edgezone_filter[0]), &sum_dead);
		ts_info("deadzone");
		goodix_set_grip_filter((int*)&(xiaomi_touch_interfaces.long_mode_value[4 * GTP_PARAMETER_NUM]),(u8*)&(dev->board_data->edge_filter_params.deadzone_filter[0]), &sum_edge);
	}

	ts_info("cornerzone");
	goodix_set_grip_filter((int*)&(xiaomi_touch_interfaces.long_mode_value[2 * 4 * GTP_PARAMETER_NUM]),(u8*)&(dev->board_data->edge_filter_params.cornerzone_filter[0]), &sum_corner);
	/* calculate checksum */
	dev->board_data->edge_filter_params.check_sum = - (sum_corner + sum_edge + sum_dead);
	ts_log("cornerzone:%*ph", 4 * GRIP_PARAMETER_NUM, dev->board_data->edge_filter_params.cornerzone_filter);
	ts_log("edgezone:%*ph", 4 * GRIP_PARAMETER_NUM, dev->board_data->edge_filter_params.edgezone_filter);
...
static int gtp_send_cur_cmd()
{
	...
	int ret = 0;
	struct goodix_ts_device *dev = goodix_core_data->ts_dev;
	u8 goodix_game_value = dev->board_data->edge_filter_params.goodix_game_value;

	ts_info("goodix_game_value = %d",goodix_game_value);
#ifdef USE_NEW_VERSION_EDGE_FILTER
	edge_filter_checksume_and_cmd(dev->board_data);
	ret = goodix_i2c_write(dev, GTP_EDGE_FILTER_PARAM_ADDR,
		dev->board_data->edge_filter_params_cmd,
		dev->board_data->edge_filter_params_cmd_length);

	if (ret < 0) {
		ts_err("send edge filter parameter failed");
	}
	ts_info("send edge filter parameters to 0x%04X", GTP_EDGE_FILTER_PARAM_ADDR);
#endif

	if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][GET_CUR_VALUE]) {
		state_data[0] = GTP_GAME_CMD;
#ifdef USE_NEW_VERSION_EDGE_FILTER
		state_data[1] = goodix_game_value & 0xCF;
#else
		state_data[1] = goodix_game_value;
#endif
		state_data[2] = 0xFF & (0 - state_data[0] - state_data[1]);
		ts_info("game: set cur write values:0x%x, 0x%x, 0x%x", state_data[0], state_data[1], state_data[2]);
		ret = goodix_i2c_write(dev, GTP_GAME_CMD_ADD, state_data, 3);
		if (ret < 0) {
			ts_err("change game mode fail");
		}
		ts_info("enable edge filter, 0x%04X", GTP_GAME_CMD_ADD);
	}
#ifdef USE_NEW_VERSION_EDGE_FILTER
	else {
		state_data[0] = GTP_GAME_CMD;
		state_data[1] = (goodix_game_value & 0xCF) | 0x10;
		state_data[2] = 0xFF & (0 - state_data[0] - state_data[1]);
		ts_info("normal: set cur write values:0x%x, 0x%x, 0x%x", state_data[0], state_data[1], state_data[2]);
		ret = goodix_i2c_write(dev, GTP_GAME_CMD_ADD, state_data, 3);
		if (ret < 0) {
			ts_err("enable edge filter failed");
		}
		ts_info("enable edge filter, 0x%04X", GTP_GAME_CMD_ADD);
	}
#endif
...
static int gtp_set_cur_value(int gtp_mode, int gtp_value)
{
...
#ifndef USE_NEW_VERSION_EDGE_FILTER
...
#endif
	if(xiaomi_touch_interfaces.touch_mode[gtp_mode][GET_CUR_VALUE] !=
					xiaomi_touch_interfaces.touch_mode[gtp_mode][SET_CUR_VALUE] || gtp_mode == Touch_Active_MODE){
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		case Touch_UP_THRESHOLD:
			...
			ts_log("i = %d  temp_value = %d [Touch_UP_THRESHOLD][GET_CUR_VALUE] = %d",i, temp_value, xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_CUR_VALUE]);
		...
		case Touch_Tolerance:
			...
			ts_log("i = %d temp_value = %d  [Touch_Tolerance][GET_CUR_VALUE] = %d",i, temp_value, xiaomi_touch_interfaces.touch_mode[Touch_Tolerance][GET_CUR_VALUE]);
...
#ifdef USE_NEW_VERSION_EDGE_FILTER
			if(temp_value == 0)
				edge_filter_corner_size = 0;
			else{
				edge_filter_corner_size =
					dev->board_data->edge_filter_corner_size[temp_value - 1];
			}
			ts_log("i = %d temp_value = %d [Touch_Edge_Filter][GET_CUR_VALUE] = %d",i, temp_value, xiaomi_touch_interfaces.touch_mode[Touch_Edge_Filter][GET_CUR_VALUE]);
#else
		...
		case Touch_Panel_Orientation:
			...
			ts_log("i = %d temp_value = %d [Touch_Panel_Orientation][GET_CUR_VALUE] = %d",i, temp_value, xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][GET_CUR_VALUE]);
			break;
		default:
			/* Don't support */
			break;
	...
	if(update){
		if(xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][GET_CUR_VALUE] == 0)
			gtp_set_edge_filter_normal();
		else
			gtp_set_edge_filter_game(edge_filter_corner_size);

		dev->board_data->edge_filter_params.goodix_game_value = goodix_game_value;
		ret = gtp_send_cur_cmd();
	}else{
		ts_info("no update");
	}
	return ret;
...
static int gtp_set_mode_long_value(int mode, int len, int *value)
{
	int ret = 0, i = 0;
	if (len == 0) {
		return -EIO;
	} else if (len > 256) {
		ts_info("too many parameters!");
		return -EINVAL;
	}

	ts_info("mode:%d, len:%d\n",mode, len);
	xiaomi_touch_interfaces.long_mode_len = len;
	for (i = 0; i < len; i++) {
		xiaomi_touch_interfaces.long_mode_value[i] = value[i];
		ts_log("xiaomi_touch_interfaces.long_mode_value:%d\n", value[i]);
	}

	if (mode == Touch_Grip_Mode) {
		if (xiaomi_touch_interfaces.touch_mode[Touch_Game_Mode][SET_CUR_VALUE]) {
			ts_info("in gamemode, don't write parameters to touch ic\n");
			return 0;
		} else{
			gtp_set_edge_filter_normal();
			ret = gtp_send_cur_cmd();
...
static int gtp_palm_sensor_write(int value)
{

	ts_info("%s value : %d\n", __func__, value);
	if (goodix_core_data) {
		goodix_core_data->palm_sensor_enabled = value;
	} else
		ts_err("%s goodix core data os NULL\n", __func__);
	return 0;
}

static char gtp_get_touch_vendor(void)
{
	char value = '2';
	ts_info("Get touch vendor: %c\n", value);
	return value;
}
...
static int gtp_reset_mode(int mode)
{
	...
	ts_info("start mode:%d", mode);
	if (mode < Touch_Report_Rate && mode > 0) {
		xiaomi_touch_interfaces.touch_mode[mode][SET_CUR_VALUE] =
		    xiaomi_touch_interfaces.touch_mode[mode][GET_DEF_VALUE];
		gtp_set_cur_value(mode,
				  xiaomi_touch_interfaces.touch_mode[mode]
				  [SET_CUR_VALUE]);
	} else if (mode == 0) {
		for (i = 0; i < Touch_Report_Rate; i++) {
			...
			gtp_set_cur_value(i,
					  xiaomi_touch_interfaces.touch_mode
					  [i][SET_CUR_VALUE]);
		}
	} else {
		ts_err("don't support");
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (core_data->debugfs) {
		...
		xiaomi_touch_interfaces.touch_vendor_read = gtp_get_touch_vendor;
		xiaomi_touch_interfaces.getModeValue = gtp_get_mode_value;
		xiaomi_touch_interfaces.setModeValue = gtp_set_cur_value;
		xiaomi_touch_interfaces.setModeLongValue = gtp_set_mode_long_value;
