...
#ifdef NFC_KERNEL_BU
		dev_warn(&nqx_dev->client->dev, "%s : NfcNciRx %x %x %x\n",
...
int nfc_ioctl_power_states(struct file *filp, unsigned long arg)
{
	...
	if (arg == 0) {
		...
		if (gpio_is_valid(nqx_dev->firm_gpio)) {
			dev_warn(&nqx_dev->client->dev, "set firm_gpio to low\n");
		...
		if (gpio_is_valid(nqx_dev->ese_gpio)) {
			if (!gpio_get_value(nqx_dev->ese_gpio)) {
				dev_warn(&nqx_dev->client->dev, "disabling en_gpio\n");
				gpio_set_value(nqx_dev->en_gpio, 0);
				usleep_range(10000, 10100);
			} else {
			...
			dev_warn(&nqx_dev->client->dev, "ese_gpio invalid, set en_gpio to low\n");
			gpio_set_value(nqx_dev->en_gpio, 0);
			usleep_range(10000, 10100);
		}
		...
		dev_warn(&nqx_dev->client->dev, "set en_gpio to high gpio\n");
		gpio_set_value(nqx_dev->en_gpio, 1);
		usleep_range(10000, 10100);
		if (nqx_dev->pdata->clk_pin_voting) {
		 ...
		 */
		dev_warn(&nqx_dev->client->dev,
			"gpio_set_value dowload mode: %s: info: %p\n",
			__func__, nqx_dev);
		...
		dev_warn(&nqx_dev->client->dev, "set en_gpio to high\n");
		gpio_set_value(nqx_dev->en_gpio, 1);
		usleep_range(10000, 10100);
		if (gpio_is_valid(nqx_dev->firm_gpio)) {
			dev_warn(&nqx_dev->client->dev, "set firm_gpio to high\n");
			gpio_set_value(nqx_dev->firm_gpio, 1);
			usleep_range(10000, 10100);
		}
		dev_warn(&nqx_dev->client->dev, "set en_gpio to low\n");
		gpio_set_value(nqx_dev->en_gpio, 0);
		usleep_range(10000, 10100);
		dev_warn(&nqx_dev->client->dev, "set en_gpio to high\n");
		gpio_set_value(nqx_dev->en_gpio, 1);
		 ...
		 */
		dev_warn(&nqx_dev->client->dev, "SN100 fw gpio HIGH\n");
		if (gpio_is_valid(nqx_dev->firm_gpio)) {
			dev_warn(&nqx_dev->client->dev, "set firm_gpio to high\n");
			gpio_set_value(nqx_dev->firm_gpio, 1);
			usleep_range(10000, 10100);
		 ...
		 */
		dev_warn(&nqx_dev->client->dev, "SN100 fw gpio LOW\n");
		if (gpio_is_valid(nqx_dev->firm_gpio)) {
			dev_warn(&nqx_dev->client->dev, "set firm_gpio to low\n");
			gpio_set_value(nqx_dev->firm_gpio, 0);
			usleep_range(10000, 10100);
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			dev_err(&client->dev,
				...
				__func__);
			is_fw_check_failed = true;
			goto err_fw_check_failed;
		}
		/* hardware dependent delay */
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata)
{
	...
	dev_warn(dev,"VEN:%u,IRQ:%u,FIRM:%u,CLKREQ:%u,ESEPWR:%u\n",pdata->en_gpio,
			pdata->irq_gpio,pdata->firm_gpio,pdata->clkreq_gpio,pdata->ese_gpio);
