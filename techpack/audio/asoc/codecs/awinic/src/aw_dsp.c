...
#define AFE_PORT_ID_AWDSP_TX   (AFE_PORT_ID_PRIMARY_MI2S_TX)  //IDX_AFE_PORT_ID_QUINARY_MI2S_TX
#define AFE_PORT_ID_AWDSP_RX   (AFE_PORT_ID_PRIMARY_MI2S_RX)  //IDX_AFE_PORT_ID_QUINARY_MI2S_RX
...
int aw_adm_param_enable(int port_id, int enable)
{
#if 0
	/*for v3*/
	int module_id = AW_COPP_MODULE_ID;
	int param_id = AW_COPP_PARAMS_ID_AWDSP_ENABLE;
	int copp_idx = 0;
...
int aw_afe_get_topology(void)
{
	return afe_get_topology(AFE_PORT_ID_AWDSP_RX);
}
...
static int aw_write_data_to_dsp(uint32_t param_id, void *data, int len)
{
	...
	while (try < AW_DSP_TRY_TIME) {
		if (aw_check_dsp_ready()) {
			...
			aw_pr_dbg("afe not ready try again");
		}
	}
	mutex_unlock(&g_aw_dsp_lock);
...
static int aw_read_msg_from_dsp(int msg_num, uint32_t msg_id, char *data_ptr, unsigned int data_size)
{
	...
	aw_pr_dbg("msg_id:0x%x, read data[%d] from dsp success", msg_id, data_size);
	mutex_unlock(&g_aw_dsp_msg_lock);
	return 0;
dsp_msg_failed:
	mutex_unlock(&g_aw_dsp_msg_lock);
...
int aw_dsp_read_te(struct aw_device *aw_dev, int32_t *te)
{
	...
	ret = aw_read_msg_from_dsp(msg_num, AW_MSG_ID_SPK_STATUS, (char *)data, sizeof(int32_t) * 8);
	if (ret) {
...
int aw_dsp_set_copp_module_en(bool enable)
{
	...
	ret = aw_adm_param_enable(AFE_PORT_ID_AWDSP_RX, enable);
	if (ret) {
		return -EINVAL;
	}
